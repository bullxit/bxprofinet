...
namespace f10.profinet.opentcp
{
    ...
    public static class S7
    {
        ...
        #region Get/Set S5Time (S5 Time) LOL
        /// <summary>
        /// Convert S5Time in Milliseconds
        /// </summary>
        /// <param name="Buffer"></param>
        /// <param name="Pos"></param>
        /// <returns>Milliseconds Remaining in Timer</returns>
        public static UInt32 GetS5TimeAt(byte[] Buffer, int Pos)
        {
            var o = (UInt16)((Buffer[Pos] << 8) | Buffer[Pos + 1]);
            string h = o.ToString("X4");

            switch (h[0])
            {
                case '0':
                    return Convert.ToUInt32(h.Substring(1)) * 10;
                case '1':
                    return Convert.ToUInt32(h.Substring(1)) * 100;
                case '2':
                    return Convert.ToUInt32(h.Substring(1)) * 1000;
                case '3':
                    return Convert.ToUInt32(h.Substring(1)) * 10000;
                default:
                    return 0;
            }
        ...
        public static void SetS5TimeAt(ref byte[] Buffer, int Pos, UInt32 Value)
        {
            string res = "0000";
            if (Value <= 9990)
            {
                res = "0" + (Value / 10).ToString();
            }
            if (Value > 9990 && Value <= 99900)
            {
                res = "1" + (Value / 100).ToString();
            }
            if (Value > 99900 && Value <= 999000)
            {
                res = "2" + (Value / 1000).ToString();
            }
            if (Value > 999000 && Value <= 9990000)
            {
                res = "3" + (Value / 10000).ToString();
            }

            UInt16 temp_int = UInt16.Parse(res, System.Globalization.NumberStyles.AllowHexSpecifier);
            var temp = BitConverter.GetBytes(temp_int);
            Buffer[0] = temp[1];
            Buffer[1] = temp[0];
    ...
    public class S7Client
    {
        ...
        public int WriteBit(int Area, int DBNumber, int Start, byte Bit, byte[] Buffer)
        {
            int BytesWritten = 0;
            return WriteArea(Area, DBNumber, (Start * 8) + Bit, 1, S7Consts.S7WLBit, Buffer, ref BytesWritten);
        }
